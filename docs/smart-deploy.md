...
> **Обратная совместимость:** команда `deploy-smart` сохранена как алиас и работает так же, как раньше, но считается устаревшей. Используйте `smart-commit deploy` — умный режим теперь включён по умолчанию.
...
# Устаревший алиас (работает, но выводит предупреждение):
